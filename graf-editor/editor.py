...
class Editor(object):
    ...
    def _create_image(self, size):
        """
        size[0] --> x coordinate (column)
        size[1] --> y coordinate (row)
        """
    ...
    def _draw_segment(self, orientation, fixed_row, begin, end, color):
        ...
        if orientation == self.valid_orientations[0]:
            self.image[fixed_row][begin : end + 1] = [color] * (end - begin)
            return

        for item in self.image[begin : end + 1]:
            item[fixed_row] = color


    def is_valid_segment(orientation, fixed_row, begin, end):
        """
